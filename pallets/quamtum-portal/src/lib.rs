...
pub mod pallet {
    ...
    use frame_system::{
        offchain::{
            ...
            SignMessage, SignedPayload, Signer, SigningTypes, SubmitTransaction,
        },
        pallet_prelude::*,
    };
    ...
    pub mod crypto {
        ...
        use crate::KEY_TYPE;
        use libsecp256k1::{Message, ECMULT_CONTEXT};
        use log::log;
        use parity_scale_codec::{Decode, Encode};
        ...
        impl frame_system::offchain::AppCrypto<MultiSigner, MultiSignature> for TestAuthId {
            ...
            fn sign(payload: &[u8], public: MultiSigner) -> Option<MultiSignature> {
                ...
                let sig2 =
                    libsecp256k1::Signature::parse_standard_slice(&sig.as_slice()[..64]).unwrap();
                let rec = &libsecp256k1::RecoveryId::parse(sig[64]).unwrap();
                let rec = ECMULT_CONTEXT
                    .recover_raw(&sig2.r, &sig2.s, rec.serialize(), &msg.0)
                    .unwrap();
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn create_finalize_transaction(c: &QuantumPortalClient) {
            c.create_finalize_transaction(4, 0, H256::zero(), &[])
                .unwrap();
        }
        ...
        fn run_mine_and_finalize(block_number: u64) {
            ...
            let lgr_mgr = ChainUtils::hex_to_address(b"d36312d594852462d6760042e779164eb97301cd");
            log::info!("000===contract address is {:?}", lgr_mgr);
            let client = ContractClient::new(rpc_endpoint.as_bytes().to_vec(), &lgr_mgr, 4);
            log::info!(
                "001===contract address is {}",
                str::from_utf8(ChainUtils::address_to_hex(client.contract_address).as_slice())
                    .unwrap()
            );
            let signer = Signer::<T, T::AuthorityId>::any_account();
            ...
            pairs
                .into_iter()
                .for_each(|p| qps.process_pair(p[0], p[1]).unwrap());
        }
    }
