...
# generate chain spec
./target/release/ferrum-x-network build-spec > ferrum-local-testnet.json
...
# start Alice node
./target/release/ferrum-x-network --chain ferrum-local-testnet.json --alice --base-path ./chain/alice --ws-port 9944

# start Bob node
./target/release/ferrum-x-network --chain ferrum-local-testnet.json --bob --base-path ./chain/bob --ws-port 9945
